...
func startProcess(k8sClient *kubernetes.Clientset, cfg *rest.Config, request *http.Request, cmd []string, ptyHandler PtyHandler) error {
	namespace := mux.Vars(request)["namespace"]
	podName := mux.Vars(request)["name"]
	containerName := mux.Vars(request)["container"]
...
func WaitForTerminal(k8sClient *kubernetes.Clientset, cfg *rest.Config, request *http.Request, sessionId string) {
